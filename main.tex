...
\setlength{\posterheaderheight}{14cm}
\setlength{\posterfooterheight}{2cm}
\setlength{\posterfooterlogoheight}{1.5cm}
\maketitle
\vspace{-0.5cm}
  ...
  \begin{column}{0.32\linewidth}
    \begin{clsblock}{Introduction}
      \begin{itemize}\setlength{\itemsep}{3pt}
        \item Rain rate estimation over the open ocean remains a challenge:
              ground radars (NEXRAD~[1]) are coastal-only; satellite
              products (IMERG~[2]) offer global coverage but at
              $\sim$8~km resolution, too coarse for fine-scale studies.
        \item The SWOT mission~[3] carries the Ka-band Radar
              Interferometer (KaRIn), a SAR at 35.75~GHz. Originally
              designed for altimetry, KaRIn is sensitive to atmospheric
              hydrometeors at \textbf{250~m/pixel}---matching NEXRAD
              resolution in range---over both coastal zones and open ocean.
        \item We present a machine learning framework to estimate
              precipitation rates from SWOT 250~m observations, trained
              against NEXRAD collocations and validated with independent
              IMERG time series.
      \end{itemize}
    \end{clsblock}
  \end{column}
  ...
  \begin{column}{0.34\linewidth}
    \begin{clsblock}{Data \& Methods}
      ...
      \begin{itemize}\setlength{\itemsep}{2pt}
        \item \textbf{7\,009} NEXRAD/SWOT patches
              (512$\times$512~px at 250~m/pixel), Aug 2023--Feb 2025.
        \item 1\,090 patches with precipitation
      ...
      \begin{itemize}\setlength{\itemsep}{2pt}
        \item U-Net (depth=4, Swish activation, BatchNorm).
        \item Four input channels: incidence-corrected $\sigma_0$ (dB),
              total coherence, incidence angle, model wind speed
              (projected from the 2~km product).
      ...
      \begin{itemize}\setlength{\itemsep}{2pt}
        ...
        \item Pixel weights decay linearly with distance to nearest
              NEXRAD station (1 at 0~km, 0 at 225~km); null-DPR pixels
      ...
      \vspace{0.1cm}
      \textbf{Ensemble.}
      Five independently trained models; consensus score from inter-model
      agreement provides a proxy for prediction confidence.
    \end{clsblock}
  \end{column}
...
\begin{columns}[T]
  ...
  \begin{column}{0.34\linewidth}
    \begin{clsblock}{Global Validation against IMERG}
      \centering
      \includegraphics[width=0.7\linewidth]{images/mollweide_pct_above_1mmh.png}

      \justifying
      \small Percentage of pixels $>$1~mm/h (1.0$^\circ$ grid,
      12\,007 passes). The ITCZ is sharply delineated in both
      Pacific and Atlantic basins, consistent with IMERG climatology.
      ...
      \end{columns}
      \justifying
      \small 21-day moving average vs.\ IMERG and ERA5. Correlation
      is high in the tropics (Pacific: 95\%, Atlantic: 75\%) but
      \textbf{negative} in the North Atlantic ($-$43\%),
      indicating difficulty with stratiform precipitation at
      higher latitudes.
    \end{clsblock}
  \end{column}
